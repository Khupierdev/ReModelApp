...
class HealthKitManager: ObservableObject {
    ...
    func handleHealthKitError(_ error: Error) {
        DispatchQueue.main.async {
            ...
            print("HealthKit Error: \(error.localizedDescription)") // Added for debugging
        }
    }
    ...
    private func fetchLatestWeight() {
        ...
        let query = HKSampleQuery(sampleType: weightType, predicate: nil, limit: 1, sortDescriptors: [NSSortDescriptor(key: HKSampleSortIdentifierStartDate, ascending: false)]) { _, results, error in
            ...
            DispatchQueue.main.async {
                print("Latest weight: \(weight) kg") // Added for debugging
            }
        }
    ...
    private func fetchLatestHeight() {
        guard let healthStore = healthStore,
              let heightType = HKObjectType.quantityType(forIdentifier: .height) else {
            handleHealthKitError(NSError(domain: "HealthKitManager", code: 4, userInfo: [NSLocalizedDescriptionKey: "No autorizado o tipo de datos no disponible"]))
            return
        }
        ...
        let query = HKSampleQuery(sampleType: heightType, predicate: nil, limit: 1, sortDescriptors: [NSSortDescriptor(key: HKSampleSortIdentifierStartDate, ascending: false)]) { _, results, error in
            ...
            DispatchQueue.main.async {
                print("Latest height: \(height) m") // Added for debugging
            }
        }
